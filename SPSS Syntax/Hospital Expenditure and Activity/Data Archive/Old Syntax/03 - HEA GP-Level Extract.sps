...
* Read in file created in first syntax.
...
AGGREGATE
 /OUTFILE=* 
  /break year recid mapcode HBRESCODE HBres LA_CODE LCAname agegroup specname SpecialtyGrp treated_board IPDC cij_pattype gpprac
...
AGGREGATE
 /OUTFILE=* 
/break year HBRESCODE HBres LCA_HB_SCOT LA_CODE LCAname agegroup specname SpecialtyGrp treated_board IPDC cij_pattype gpprac
/Bed_Days = sum(Bed_Days)
/Total_Net_Cost = sum(Total_Net_Cost)
/Episodes = sum(Episodes).
execute.
...
if hbres ='NHS Orkney' and LCAname ne 'Orkney' flag =1. 	 	 	 	 	 	 
if hbres ='NHS Shetland' and LCAname ne 'Shetland' flag =1. 	 	 	 	 	 	 
...
save outfile = !file + 'PLICS_Prac.zsav'
    /zcompressed. 

get file =   !file + 'PLICS_Prac.zsav'. 
...
match files file = *
   /table= '/conf/irf/11-Development team/Dev08-Pathways/Tableau Matrix/Practice-Cluster Details.sav'
   /by Practice.
execute.

save outfile =  !file + 'PLICS_Prac' + !year + '.zsav'
    /drop Board HSCP HSCP_Code
    /zcompressed.

get file =  !file + 'PLICS_Prac' + !year + '.zsav'.

******************************************
* This section should only be run after every financial year's data has been finalised
*Add files together.

add files file =  !file + 'PLICS-Main201516.sav'
 /file = !file + 'PLICS-Main201617.sav'
 /file = !file + 'PLICS-Main201718.sav'
 /file = !file + 'PLICS-Main201819.sav'.

alter type location(a7).
alter type prac (a25).

save outfile = !file + 'PLICS_Final1.zsav'
    /zcompressed.

add files file= !file + 'PLICS_Prac201516.zsav'
 /file = !file + 'PLICS_Prac201617.zsav'
 /file = !file + 'PLICS_Prac201718.zsav'
 /file = !file .
execute.

save outfile = !file + 'PLICS_Final2.zsav'.

add files file = !file + 'PLICS_Final1.zsav'
    /file = !file + 'PLICS_Final2.zsav'.
execute.

alter type prac(f5.0).
alter type year(f4.0).

save outfile =  !file + 'PLICS-Final.sav'. 

get file =  !file + 'PLICS-Final.sav'. 
