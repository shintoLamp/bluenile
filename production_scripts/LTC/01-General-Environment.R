...
# Constants used in the HEA workbook
hea_constants <- list(
  health_boards = c(
...
# Constants used in any LTC-related workbook
ltc_group_names <- c("cardiovascular", "neurodegenerative", "respiratory", "other_organs", "other_ltcs", "no_ltc")
ltc_names <- c("cvd", "copd", "dementia", "diabetes", "chd", "hefailure", "refailure", "epilepsy",
...
age_groups_cut <- function(df, age_variable) {
  return_df <- df %>% 
    dplyr::mutate(age_grp = cut({{age_variable}}, breaks = c(-1, 17, 44, 64, 74, 84, max(age)),
                           labels = c("0-17", "18-44","45-64", "65-74", "75-84", "85+")))
  return(return_df)
}
...
clacks_stirling_pivot <- function(lcaname, temp_lca) {
