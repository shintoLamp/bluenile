...
DEFINE !NRAC()
'/conf/sourcedev/TableauUpdates/Hospital Expenditure and Activity/Development and Checking/2020 syntax reorganise/NRACLCA.sav'
!ENDDEFINE.
...
save outfile = !file + 'HEATestTemp' + !year + '.sav' /zcompressed.
get file = !file + 'HEATestTemp' + !year + '.sav'.
...
add files file = *
/file = !file + 'HEATestTemp' + !year + '.sav'.
save outfile = !file + 'HEATestTemp' + !year + '.sav' /zcompressed.
get file = !file + 'HEATestTemp' + !year + '.sav'.
...
save outfile = !file + 'HEAMaster' + !year + '.sav' /zcompressed.
...
get file = !file + 'HEAMaster' + !year + '.sav'.
...
if lcaname = 'Clackmannanshire & Stirling' LA_code = 'S00000015'.
...
/Total_Net_Cost = sum(Total_Net_Cost)
/Episodes = sum(Episodes)
/Population = first(popn)
/FlagGroupSpecs = max(FlagGroupSpecs)
/FlagAllSpecialties = max(FlagAllSpecialties)
/FlagDelegatedSpec = max(FlagDelegatedSpec).

* We create the rate measures here to simplify processing in Tableau.
 * numeric BedDaysPer1000(f8.2) EpisodesPer1000(f8.2) ExpenditurePer1000(f8.2).
 * if bed_days ne 0 BedDaysPer1000 = (bed_days/population)*1000.
 * if Episodes ne 0 EpisodesPer1000 = (Episodes/population)*1000.
 * if Total_Net_Cost ne 0 ExpenditurePer1000 = (Total_Net_Cost/population)*1000.
...
save outfile = !file + 'HEAHospital' + !year + '.sav'.

erase file = !file + 'HEATemp' + !year + '.sav'.
erase file = !file + 'HEAMaster' + !year + '.sav'.
