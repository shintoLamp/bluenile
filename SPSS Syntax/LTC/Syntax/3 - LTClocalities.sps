...
* need to run for last four financial years.
Define !year()
'201920'
!Enddefine. 
...
* Run on sourcedev.
Define !file()
'/conf/sourcedev/TableauUpdates/LTC/Outputs/201920/'
!Enddefine. 
...
*Create age groups. 
String agegroup (a7).
recode age (0 thru 17='0-17')(18 thru 44='18-44')(45 thru 64= '45-64')(65 thru 74= '65-74')(75 thru 84='75-84')(85 thru hi = '85+')(sysmis='Unknown') into agegroup.
...
*************************************************************************************. Okay, 337 localities missing.
*Only required after 14/15.
delete variables locality. 
...
aggregate outfile=* 
 /break CHI HBRESCODE lca gender sex agegroup Patient_Type locality
...
************************************************* No localities missing.

*Part 1 all LTCs.
temporary.
compute type= 'Any1+'.
aggregate outfile = !file + 'tempM1.sav' 
 /BREAK hbrescode lca agegroup gender type locality Patient_Type 
 /count= sum(LTC)
 /Cost_Total_Net= sum(Cost_Total_Net).
execute.
...
compute gender='All'.
aggregate outfile= !file + 'tempM1.sav' 
 /break hbrescode lca gender agegroup type locality Patient_Type 
 /count= sum(count)
 /Cost_Total_Net= sum(Cost_Total_Net).
execute. 
 ...
 * get file = '/conf/irf/05-lookups/04-geography/LCA_lookup.sav'.
 * compute LCAcode = LTRIM(LCAcode).
 * execute.
 * if char.length(LCAcode) = 1 LCAcode = concat('0', LCAcode).
 * execute.
 * save outfile = '/conf/sourcedev/TableauUpdates/LTC/Outputs/LCAlookup.sav' /zcompressed.
 * get file = !file + 'LCAlookup.sav'.
...
match files file=*
 /table =  '/conf/sourcedev/TableauUpdates/LTC/Outputs/LCAlookup.sav'
 /by lcacode.
execute. 

* Dropped many LCA names.
...
match files file=*
 /table = !file + 'Locality' + !year + '.sav' 
 /by locality sex agegroup.
exe.
...
save outfile= !file + 'LTClocality' + !year + '.sav'
/zcompressed.

get file= !file + 'LTClocality' + !year + '.sav'.
