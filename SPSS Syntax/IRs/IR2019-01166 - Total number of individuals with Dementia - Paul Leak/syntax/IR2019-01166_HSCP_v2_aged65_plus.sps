...
*Select Individuals aged 65+.
string agegroup (a25).
if age ge 65 agegroup='65+ years'.
execute.
...
save outfile = !output + 'Total Individuals aged 65+ with Dementia by agegroup gender HSCP for 201718.sav' 
  /keep hscp agegroup gender_type tot_individuals.
 
get file = !output + 'Total Individuals aged 65+ with Dementia by agegroup gender HSCP for 201718.sav'. 


*Aggregate total individuals aged 65+ by gender for each hscp .
aggregate outfile =*
 /break hscp gender_type 
  /tot_individuals = sum(tot_individuals).
execute.

save outfile = !output + 'Total Individuals aged 65+ with Dementia by gender HSCP for 201718.sav'. 

get file = !output + 'Total Individuals aged 65+ with Dementia by agegroup gender HSCP for 201718.sav'. 

select if agegroup ne 'Unknown'.
execute.

sort cases by gender_type(d).

save outfile = !output + 'Total Individuals aged 65+ with Dementia by agegroup gender HSCP for 201718.sav'. 

save translate outfile = !output + 'Total Individuals aged 65+ with Dementia by agegroup gender HSCP for 201718.xlsx' 
