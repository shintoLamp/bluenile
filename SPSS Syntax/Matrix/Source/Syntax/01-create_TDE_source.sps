...
get file = !hscdiip_slf + "source-individual-file-20" + !FY + ".zsav"
    /Drop Year DoB Postcode health_net_costincDNAs health_net_costincIncomplete HL1_in_FY
...
* Remove non-Scottish and reduce to keep pop.
select if lca NE "".
select if Keep_Population = 1.

string Partnership (a25).
compute Partnership = valuelabels(lca).
...
compute Delayed_Episodes = DD_NonCode9_episodes + DD_Code9_episodes.
compute Delayed_Beddays = DD_NonCode9_beddays + DD_Code9_beddays.
compute GP_Contacts = OoH_homeV + OoH_advice + OoH_DN + OoH_NHS24 + OoH_other + OoH_PCC.

* Because of how delays are matched there are sometimes errors in bedday counts.
If Delayed_Beddays GT Total_Beddays Delayed_Beddays = Total_Beddays.
...
if (DD_NonCode9_episodes GT 0
    OR DD_Code9_episodes GT 0) Delayed_Patients = 1.
if (DN_Contacts GE 1) DN_Patients = 1.
if (CH_Admissions GE 1) CH_Patients = 1.
if (GP_Contacts GE 1) GP_Patients = 1.
...
string HHG_Risk_Group (A8).
Do if Range(HHG_End_FY, 0, 39).
    Compute HHG_Risk_Group = "0 to 39".
Else if Range(HHG_End_FY, 40, 59).
    Compute HHG_Risk_Group = "40 to 59".
Else if Range(HHG_End_FY, 60, 79).
    Compute HHG_Risk_Group = "60 to 79".
Else if Range(HHG_End_FY, 80, 100).
    Compute HHG_Risk_Group = "80+".
End if.
If HHG_Risk_Group = "" HHG_Risk_Group = "N/A".
...
* Match on CHI number so we can use the demographic lookups.
match files file = *
...
* Temporarily getting rid of this cohort.
If Demographic_Cohort = "Assisted Living in the Community" Demographic_Cohort = "Healthy and Low User".

if NSU = 1 AND Demographic_Cohort = "Healthy and Low User" Demographic_Cohort = "Non-Service User".

sort cases by Anon_CHI.

save outfile = !sourcdev_matrix_temp + "Patient_Aggregate_" + !version + "_20" + !FY + ".zsav"
    /Keep
    Anon_chi
    arth
    asthma
    ...
    Total_Beddays
    Unplanned_Beddays
    Hospital_Elective_Cost
    Hospital_Emergency_Cost
    Maternity_Cost
    Hospital_Elective_Beddays
    Hospital_Emergency_Beddays
    Delayed_Episodes
    Delayed_Beddays
    GP_Contacts
    ...
    Urban_rural
    Gender
    Hospital_Elective_attendance
    Hospital_Emergency_attendance
    Maternity_attendance
    CH_beddays
    CH_cost
    DN_cost
    DN_contacts
...
*********************************************.
get file = !sourcdev_matrix_temp + "Patient_Aggregate_" + !version + "_20" + !FY + ".zsav".
...
aggregate outfile = *
    /BREAK Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender HHG_Risk_Group
    /NoPatients = n
    ...
    /CH_Admissions = sum(CH_Admissions)
    /CH_Beddays = sum(CH_Beddays)
    /CH_Cost = sum(CH_Cost)
    /DN_Cost=sum(DN_Cost)
    /DN_Contacts = sum(DN_Contacts)
    /GP_Contacts = sum(GP_Contacts)
    /GP_Cost = sum(GP_Cost)
    ...
    /CH_Patients = sum(CH_Patients)
    /DN_Patients = sum(DN_Patients)
    /GP_Patients = sum(GP_Patients)
    /ZeroLTC = sum(ZeroLTC)
    /OneLTC = sum(OneLTC)
    /TwoLTC = sum(TwoLTC)
...
save outfile =  !sourcdev_matrix_temp + "National_TDE_AE2_" + !version + "_20" + !FY + ".zsav"
    /zcompressed
    /Keep Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender
    HHG_Risk_Group NoPatients Total_Cost Total_Beddays Total_Attendances Data Service_Area.
...
select if CH_Cost GT 0 OR CH_Patients GT 0 OR CH_Admissions GT 0 OR CH_Beddays GT 0.

compute Total_Cost = $sysmis.
compute Total_Beddays = CH_Beddays.
compute Total_Attendances = CH_Admissions.
compute NoPatients = CH_Patients.

string Service_Area (A30).
compute Service_Area = "Care Home".

save outfile =  !sourcdev_matrix_temp + "National_TDE_CH_" + !version + "_20" + !FY + ".zsav"
    /zcompressed
    /Keep Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender
    HHG_Risk_Group NoPatients Total_Cost Total_Beddays Total_Attendances Data Service_Area.
...
select if DN_Cost GT 0 OR DN_Patients GT 0 OR DN_Contacts GT 0.

compute Total_Cost = $sysmis.
compute Total_Beddays = $sysmis.
compute Total_Attendances = DN_Contacts.
compute NoPatients = DN_Patients.

string Service_Area (A30).
compute Service_Area = "District Nursing".

save outfile =  !sourcdev_matrix_temp + "National_TDE_DN_" + !version + "_20" + !FY + ".zsav"
    /zcompressed
    /Keep Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender
    HHG_Risk_Group NoPatients Total_Cost Total_Attendances Data Service_Area.
...
select if GP_Cost GT 0 OR GP_Patients GT 0 OR GP_Contacts GT 0.

compute Total_Cost = GP_Cost.
compute Total_Beddays = $sysmis.
compute Total_Attendances = GP_Contacts.
compute NoPatients = GP_Patients.

string Service_Area (A30).
compute Service_Area = "GP Out of Hours".

save outfile =  !sourcdev_matrix_temp + "National_TDE_GPOoH_" + !version + "_20" + !FY + ".zsav"
    /zcompressed
    /Keep Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender
    HHG_Risk_Group NoPatients Total_Cost Total_Attendances Data Service_Area.
...
add files file =  !sourcdev_matrix_temp + "National_TDE_AE2_" + !version + "_20" + !FY + ".zsav"
    ...
    /file =  !sourcdev_matrix_temp + "National_TDE_DN_" + !version + "_20" + !FY + ".zsav"
    /file =  !sourcdev_matrix_temp + "National_TDE_GPOoH_" + !version + "_20" + !FY + ".zsav"
    /file =  !sourcdev_matrix_temp + "National_TDE_CH_" + !version + "_20" + !FY + ".zsav".
...
Erase file =  !sourcdev_matrix_temp + "National_TDE_CH_" + !version + "_20" + !FY + ".zsav".
Erase file =  !sourcdev_matrix_temp + "National_TDE_DN_" + !version + "_20" + !FY + ".zsav".
Erase file =  !sourcdev_matrix_temp + "National_TDE_GPOoH_" + !version + "_20" + !FY + ".zsav".
...
save outfile = !sourcdev_matrix_temp + "National_Arthritis_" + !version + "_20" + !FY + ".zsav"
    /zcompressed
    /Keep Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender HHG_Risk_Group
...
save outfile = !sourcdev_matrix_final + !version + "_TDE_20" + !FY + ".zsav"
    /Keep Partnership Locality Service_Use_Cohort Demographic_Cohort SIMD_Quintile ResourceGroup AgeBand Urban_Rural LTC_Total Gender
    HHG_Risk_Group NoPatients Total_Cost Total_Beddays Unplanned_Beddays Total_Admissions AE2_Attendances Outpatient_Attendances Preventable_Admissions Preventable_Beddays
    ...
    arth asthma atrialfib cancer copd cvd dementia diabetes epilepsy chd hefailure liver ms parkinsons refailure Data Total_Attendances Service_Area LTC_Name Demograph_Name Year
    Delayed_Episodes Delayed_Beddays Delayed_Patients
    /zcompressed.
get file = !sourcdev_matrix_final + !version + "_TDE_20" + !FY + ".zsav".

* Housekeeping.
Erase file !sourcdev_matrix_temp + "Patient_Aggregate_" + !version + "_20" + !FY + ".zsav".
