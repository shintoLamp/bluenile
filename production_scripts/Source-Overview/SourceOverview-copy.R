...
#overview <- master %>% managed_lookup(dz, matchby = "datazone2011") #changed this line for the one below (line 46) due to the following error:
#Error in managed_lookup(., dz, matchby = "datazone2011") : 
#could not find function "managed_lookup"

overview <- left_join(master, dz, by = "datazone2011")
# Match hospital location codes to get location names
#overview %<>% managed_lookup(hosp, matchby = c("location" = "Location"))
overview <- left_join(overview, hosp, by = c("location" = "Location"))

# Match specialty codes to get their descriptions
#overview %<>% managed_lookup(spec, matchby = c("spec" = "Speccode")) %>%
overview <- left_join(overview, spec, by = c("spec" = "Speccode")) %>%
...
#Rename ca2018 for the filters to work 
source_overview <- source_overview %>% 
  rename(LA_CODE = ca2018) 
